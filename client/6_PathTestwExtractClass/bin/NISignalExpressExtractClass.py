...
class NISignalExpressExtract:
	...
	print "NISignalExpressExtract"
        self._meta_voltage_filename = os.path.join(file_location, "Voltage_meta.txt")
        self._meta_current_filename = os.path.join(file_location, "Current_meta.txt")
        self._tdms_voltage_filename = os.path.join(file_location, "Voltage.tdms")
        self._tdms_current_filename = os.path.join(file_location, "Current.tdms")
        self._channel_names = []
    ...
    def calibrate_output(self):
        ...
        writer.writerow(new_row)
        for row in reader:
        	value = self.calibrate_value(row[1], row[2])
